...
def download_video_segment(video_url, start_time, end_time, output_format):
    # Function to download and process the video segment
    ydl_opts = {
        'format': 'bestvideo+bestaudio',
        'quiet': True,
        'forcefilename': True,
        'outtmpl': './outputs/video-cut/%(title)s.%(ext)s',  # Output template
    }
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        video_filename = f"./outputs/video-cut/{video_title}.mkv" if video_title else f"./outputs/video-cut/output.mkv"

    # Cut the downloaded video file using FFmpeg
    output_filename = f"./outputs/video-cut/{video_title}_cut.{output_format}" if video_title else f"./outputs/video-cut/output_cut.{output_format}"
    start_time = convert_time_to_seconds(start_time)
    end_time = convert_time_to_seconds(end_time)
    subprocess.run(['ffmpeg', '-i', video_filename, '-ss', str(start_time), '-to', str(end_time), '-c', 'copy', output_filename])
...
def main():
    while True:
        ...
        print("1: Lossless MKV (Higher Quality, Big Size)")
        print("2: MP4 (Good Quality, Smaller Size)")
        ...
        os.makedirs('./outputs/video-cut', exist_ok=True)

        download_video_segment(video_url, start_time, end_time, output_format)
